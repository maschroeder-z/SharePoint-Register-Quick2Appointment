...
export default class QuickAppointmentRegisterApplicationCustomizer
        ...
        if (listGuid !== null && itemID>0)
        {
          ...
          else
          {
            ...
            btnRegister.onclick = async (source:any) => {                
              ...
              Dialog.alert(strings.MSGRegistered).then(() => {
                location.reload();                 
              });                            
            }        
          }
  ...
  private loadAppointment(listGuid:string, id: number):Promise<IEvent> {
    let endpoint = `${this.context.pageContext.web.absoluteUrl}/_api/web/lists/getById('${listGuid}')/items(${id})?$expand=ParticipantsPicker&$select=Id,Title,ParticipantsPicker/ID,ParticipantsPicker/Title`;
